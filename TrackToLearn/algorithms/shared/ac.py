...
class PolicyGradient(nn.Module):
    ...
    def act(
        self, state: torch.Tensor, stochastic,
    ) -> torch.Tensor:
    ...
    def select_action(
        self, state: np.array, stochastic=False,
    ) -> np.ndarray:
...
class Critic(nn.Module):
    ...
    def forward(self, state) -> torch.Tensor:
        ...
        v1 = torch.tanh(self.l1(state))
        v1 = torch.tanh(self.l2(v1))
        v1 = self.l3(v1)
...
class SharedActorCritic(nn.Module):
    ...
    def _mu(self, shared: torch.Tensor):
        p = self.actor(shared)
        return p

    def _distribution(self, shared: torch.Tensor):
        mu = self._mu(shared)
        std = torch.exp(self.log_std)
        return Normal(mu, std)

    def _critic(self, shared: torch.Tensor):
        p = self.critic(shared)
        return p
    ...
    def act(
        ...
        shared = self.__call__(state)
        pi = self._distribution(shared)
        action = pi.sample() if stochastic else pi.mean
