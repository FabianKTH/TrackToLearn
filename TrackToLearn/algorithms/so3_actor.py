...
class SO3Actor(nn.Module):
    ...
    def _init_spharm_basis(self):
        ...
        # v = pg.graphs.SphereIcosahedral(subdivisions=4).coords
        ...
        v_small = pg.graphs.SphereIcosahedral(subdivisions=2).coords  # (1) no subdivisions
        self.B_desc_small, self.invB_desc_small = self._init_dipy_basis(basis='descoteaux07',
    ...
    def _init_deepsphere(self):
        ...
        self.conv0 = SphericalChebBN(2, 4, self.laps[4].to(device=device), kernel_size=3)
        ...
        #                                  pooling=self.pooling_class.pooling, kernel_size=3)
        self.conv3 = FinalSphericalChebBNPool(16, 2, self.laps[1].to(device=device),
                                              pooling=self.pooling_class.pooling, kernel_size=3)
    ...
    def _reformat_state(self, state):
        ...
        dir_part = self._sph_to_sp(dir_part, sph_basis='descoteaux')  # spharm?
    ...
    def _so3_deepsphere_net(self, signal):
        x = torch.cat([signal[:, 0, None], signal[:, -4, None]], 1)
        # x = signal[:, 0, None]
    ...
    def forward(
        ...
        p = torch.matmul(p, self.invB_desc_small)
        ...
        """
        stochastic = False  # trash
        mu = self._get_direction(state[:, 0, None], is_sp_signal=True)[:, 0]  # trash
        std = torch.ones_like(mu)  # trash
        """
